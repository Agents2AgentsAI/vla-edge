...
class CameraClient:
    ...
    def __init__(
        ...
        max_frame_age_sec: float | None = 0.5,
    ) -> None:
    ...
    def _connect(self) -> None:
        if self._sock is not None:
            try:
                ...
                logger.debug("Failed to close stale camera socket", exc_info=True)
...
class CameraSubscriber:
    ...
    def try_recv(self) -> dict[str, np.ndarray] | None:
        ...
        except Exception as exc:  # noqa: BLE001  # Drop malformed publishes.
    ...
    def close(self) -> None:
        try:
            ...
            logger.debug("Failed to close camera subscriber", exc_info=True)


def _display_available() -> bool:
    """Return whether OpenCV's Linux Qt backend has a display to use."""
    if not sys.platform.startswith("linux"):
        return True
    display = os.environ.get("DISPLAY")
    xdpyinfo = shutil.which("xdpyinfo")
    if not display or xdpyinfo is None:
        return False
    try:
...
def _ordered_camera_names(frames: dict[str, np.ndarray]) -> list[str]:
...
def _run_browser_viewer(pub_endpoint: str) -> int:
    from mjpeg_viewer import main as browser_main

    return browser_main(["--pub-endpoint", pub_endpoint])
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="auto opens a local window when possible and otherwise serves "
             "the stream at http://<host>:8090/.",
    )
    args = parser.parse_args(argv)

    view = args.view
    if view == "auto":
        view = "window" if _display_available() else "browser"
    if view == "window" and not _display_available():
        parser.error(
            "--view window needs a graphical display; use --view browser or "
            "reconnect with X forwarding"
        )
    if view == "browser":
        if args.mode != "sub":
            parser.error("the browser viewer requires --mode sub")
        if args.view == "auto":
            print(
                "[camera_client] No graphical display detected; starting the "
                "browser viewer."
            )
        return _run_browser_viewer(args.pub_endpoint)
    ...
    print(
        f"[camera_client] mode={args.mode}, press 'q' in the window to quit.",
        flush=True,
    )
    try:
        while True:

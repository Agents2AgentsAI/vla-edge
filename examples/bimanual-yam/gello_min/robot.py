...
class Robot(Protocol):
    ...
    @abstractmethod
    def close(self) -> None:
        """Stop the controller and release the robot hardware."""
        raise NotImplementedError
    ...
    def get_observations(self) -> dict[str, np.ndarray]:
        ...
        for k in l_obs:
            try:
                ...
                raise RuntimeError(
                    f"failed to concatenate observation {k!r}: "
                    f"left shape={np.shape(l_obs[k])}, right shape={np.shape(r_obs[k])}"
                ) from exc

        return return_obs

    def close(self) -> None:
        """Close both arms even if one arm reports a shutdown error."""
        errors = []
        for robot in (self._robot_l, self._robot_r):
            try:
                robot.close()
            except Exception as exc:  # noqa: BLE001 - still close the other arm
                errors.append(exc)
        if errors:
            raise RuntimeError(f"failed to close {len(errors)} arm(s)") from errors[0]
